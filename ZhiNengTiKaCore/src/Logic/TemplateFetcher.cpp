...
{
    const TemplateSummary templateSummary(QString(), templateCode.trimmed());
    handleTemplateRequest(templateSummary);
}

void TemplateFetcher::handleTemplateRequestNetwork(const TemplateSummary &templateSummary)
{
    ...
    hash.insert(reply, templateAnalysis);
...
void TemplateFetcher::onHandleTemplateReplyFinished()
{
    ...
    reply->deleteLater();
    TemplateAnalysis templateAnalysisfromNetwork(hash.take(reply));
    if (rawData.isEmpty())
    {
